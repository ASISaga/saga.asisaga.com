...
export function setupBrain(scene, coreRadius) {
  ...
  const brainGeometry = new THREE.BufferGeometry().setFromPoints(brainCurve.map(v => v.clone().multiplyScalar(coreRadius * 1.10)));
  ...
  // Complex neural network: 24 neurons randomly distributed inside the brain volume
  ...
  ).multiplyScalar(coreRadius * 1.10);
